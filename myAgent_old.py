...
trainingSchedule = [("random", 20), ("self", 5)]
...
class MyCreature:
    ...
    def __init__(self):
        ...
        # values
        self.chromosome=np.random.rand(5, 75)
        # .
        # .
        # .
    ...
    def AgentFunction(self, percepts):
        ...
        count = 0
        for c in range(0,5):
                actions[c] = np.sum(self.chromosome[c,:] * percepts.flatten())
...
def newGeneration(old_population):
    ...
    for n, creature in enumerate(old_population):
        ...
        fitness[n] = creature.size + creature.turn + creature.bounces
        if(creature.alive):
            fitness[n] + 10
    print(fitness)
